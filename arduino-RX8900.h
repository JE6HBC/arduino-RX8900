...
void RegisterRead(void)
{
  ...
  Wire.endTransmission(false);
  Wire.requestFrom(RX8900A_ADRS, 2);//Backup_Function_reg Å` TEMP_reg
  TEMP = Wire.read();               //0x17
...
loop7:
  ...
  Serial.println();
  Serial.println("SET ALARM HOUR(0Å`23)");
  while(INPUT_HOUR() == -1);
  ByteWrite(HOUR_Alarm_reg,HOUR);
  Serial.println();
  Serial.println("SET ALARM MINUTE(0Å`59)");
  while(INPUT_MIN() == -1);
...
{
  unsigned int  ui;
  Serial.println("DOWN TIMER MODE (0 min 1 sec Å` 59 min 59 sec) ");
  Serial.println("SET MINUTE (0Å`59) and 'CR'");
  while(Serial.available() > 0)ui = Serial.read();
  while(INPUT_MIN() == -1);
  ui = ((((MIN >> 4)& 0x0F) * 10) + (MIN & 0x0F)) * 60;
  Serial.println("SET SECOND (0Å`59) and 'CR'");
  while(INPUT_MIN() == -1);
